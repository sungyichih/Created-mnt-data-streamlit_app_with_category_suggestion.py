...
def read_excel_raw(uploaded_file, sheet_name=0):
    ...
    return pd.read_excel(uploaded_file, sheet_name=sheet_name, header=None, engine=engine)

def _normalize_header(value):
    return normalize_text(value).upper()

def _find_header_row(df, required_headers, max_scan_rows=20):
    """Scan the first rows for a header row containing all required_headers.

    required_headers maps an output key to a set of acceptable normalized
    header strings. Returns (header_row_idx, {key: col_idx}, normalized_row)
    or (None, None, None) if no row matches all required headers.
    """
    for row_idx in range(min(max_scan_rows, len(df))):
        row_values = [_normalize_header(v) for v in df.iloc[row_idx].tolist()]
        column_map = {}
        for key, candidates in required_headers.items():
            ...
            if match_idx is None:
                column_map = None
                break
            column_map[key] = match_idx
        if column_map is not None:
            return row_idx, column_map, row_values
...
def read_organized_cpn_mapping(uploaded_file, sheet_name=0):
    df = read_excel_raw(uploaded_file, sheet_name)

    required_headers = {
        'SPN': {'MATERIAL'},
        'CPN': {
            'CUSTOMER MATERIAL NO.', 'CUSTOMER MATERIAL NO',
            "CUSTOMER'S MATERIAL NO.", "CUSTOMER'S MATERIAL NO",
        },
    }
    header_row_idx, column_map, _ = _find_header_row(df, required_headers)
    if header_row_idx is None:
        raise ValueError(
            f'Cannot find a header row with "Material" and "Customer material no." '
            f'columns in file "{uploaded_file.name}".'
        )

    data = df.iloc[header_row_idx + 1:].reset_index(drop=True)
    out = pd.DataFrame({
        'SPN': data.iloc[:, column_map['SPN']].apply(normalize_text),
        'CPN': data.iloc[:, column_map['CPN']].apply(normalize_text),
    })
    out = out[(out['SPN'] != '') & (out['CPN'] != '')].drop_duplicates().reset_index(drop=True)
    return out

def read_organized_mpn_mapping(uploaded_file, sheet_name=0):
    df = read_excel_raw(uploaded_file, sheet_name)

    required_headers = {
        'SPN': {'MATERIAL NO', 'MATERIAL NO.', 'MATERIAL NUMBER'},
        'MFG': {'MFR. NAME', 'MFR NAME', 'MANUFACTURER NAME'},
        'MPN': {'MFR. P/N', 'MFR P/N', 'MANUFACTURER P/N', 'MFR. PART NO.'},
    }
    header_row_idx, column_map, header_values = _find_header_row(df, required_headers)
    if header_row_idx is None:
        raise ValueError(
            f'Cannot find a header row with "Material NO", "MFR. Name", and "MFR. P/N" '
            f'columns in file "{uploaded_file.name}".'
        )
    ...
    out = pd.DataFrame({
        'SPN': data.iloc[:, column_map['SPN']].apply(normalize_text),
        'System_MFG': data.iloc[:, column_map['MFG']].apply(normalize_text),
        'System_MPN': data.iloc[:, column_map['MPN']].apply(normalize_text),
    })

    # "Customer O" and "OSE OUT" columns: the system marks a part "X" instead
    # of deleting it when that MPN is no longer allowed for use. Either
    # column being "X" means this MPN is crossed out.
    cross_headers = {'CUSTOMER O', 'OSE OUT'}
    cross_col_idxs = [i for i, v in enumerate(header_values) if v in cross_headers]

    is_crossed = pd.Series(False, index=out.index)
    for col_idx in cross_col_idxs:
        is_crossed = is_crossed | (data.iloc[:, col_idx].apply(normalize_key) == 'X')
    out['Is_Crossed'] = is_crossed
...
def map_cpn_to_spn(original_base_df, original_mpn_df, cpn_mapping_df, system_mpn_df):
    ...
    # Crossed (X-marked) MPNs are treated as no longer existing in the system,
    # so they don't count toward MPN-overlap-based SPN selection.
    sys_groups = {}
...
def build_mpn_compare(mapped_df, original_mpn_df, system_mpn_df):
    ...
    # Split system MPNs into still-valid vs crossed (X-marked / no longer usable)
    # per SPN, so crossed MPNs don't count as "in System_MPN_List" for matching.
    sys_groups = {}
    crossed_groups = {}
    for _, row in system_mpn_df.iterrows():
        spn = normalize_text(row['SPN'])
...
def make_result_excel(original_base_df, original_mpn_df, mapped_df, compare_df, missing_spn_df, summary_df):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        purple_fill = PatternFill(fill_type='solid', start_color='B4A7D6', end_color='B4A7D6')
...
st.caption(
    ...
    '- System CPN file: first sheet, with "Material" and "Customer material no." columns\n'
    '- System MPN file: first sheet, with "Material NO", "MFR. Name", "MFR. P/N" columns'
)

st.info(
    'Please prepare files as follows:\n'
    '• Original BOM: sheet name "BOM"\n'
    '• System CPN export: first sheet must contain "Material" and "Customer material no." columns\n'
    '• System MPN export: first sheet must contain "Material NO", "MFR. Name", "MFR. P/N" columns '
    '("Customer O" / "OSE OUT" = "X" marks an MPN as crossed/not usable)'
)
...
if process:
    if original_bom_file is None or organized_cpn_file is None or organized_mpn_file is None:
        ...
        try:
            ...
            cpn_mapping_df = read_organized_cpn_mapping(organized_cpn_file)
            mpn_mapping_df = read_organized_mpn_mapping(organized_mpn_file)
            ...
            m4.metric('MPN Crossed in System', int((compare_df['MPN_Compare_Status'] == 'MPN Crossed in System').sum()))
            diff_count = int(
                compare_df['MPN_Compare_Status'].isin(
                    ['Partial Match', 'Missing in System', 'Extra in System', 'Missing SPN', 'Ambiguous - same overlap', 'MPN Crossed in System']
                ).sum()
            )
            m5.metric('MPN differences', diff_count)
