...
def read_organized_mpn_mapping(uploaded_file, sheet_name='SPN-MPN Mapping'):
    ...
    # Columns H and I: the system marks a part "X" instead of deleting it when
    # that MPN is no longer allowed for use. Either column being "X" means crossed.
    if df.shape[1] > 7:
        col_h = df.iloc[:, 7].apply(normalize_key)
    else:
        col_h = pd.Series([''] * len(df), index=df.index)
    if df.shape[1] > 8:
        col_i = df.iloc[:, 8].apply(normalize_key)
    else:
        col_i = pd.Series([''] * len(df), index=df.index)
    out['Is_Crossed'] = (col_h == 'X') | (col_i == 'X')
...
st.info(
    ...
    '• Organized MPN file: "SPN-MPN Mapping" (columns H/I: "X" marks an MPN as crossed/not usable)'
)
